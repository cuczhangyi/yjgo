...
func (f *UploadFile) Save(dirPath string, randomlyRename ...bool) (filename string, err error) {
	...
	if len(randomlyRename) > 0 && randomlyRename[0] {
		name = strings.ToLower(strconv.FormatInt(gtime.TimestampNano(), 36) + grand.Digits(6))
...
func  GetUploadFile( r *ghttp.Request,name string) *UploadFile {
	uploadFiles := GetUploadFiles(r,name)
...
//
// Note that the <name> is the file field name of the multipart form from client.
func  GetUploadFiles(r *ghttp.Request, name string) UploadFiles {
